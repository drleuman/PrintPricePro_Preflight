...
// 0.1) Gemini Proxy (Used by AI Audit Modals)
...
if (!global.__SERVER_STARTED) {
  ...
  const runDiagnostics = () => {
    ...
    // P0: Always initialize schema if possible
    if (startupErrors.length === 0) {
      debugLog('Initializing Identity & Governance Schema...');
      initSchema()
        .then(ok => {
          if (ok) debugLog('Identity Node: Schema Sync OK');
          else debugLog('Identity Node: Schema Sync FAILED');
        })
        .catch(err => console.error('[FATAL] Schema init error:', err));
    }
