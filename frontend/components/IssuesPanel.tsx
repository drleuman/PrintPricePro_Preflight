...
export const IssuesPanel: React.FC<Props> = ({
  ...
  const Row = ({ index, style }: { index: number; style: React.CSSProperties }) => {
    ...
    const categoryLabel = (ISSUE_CATEGORY_LABELS[displayCategory as keyof typeof ISSUE_CATEGORY_LABELS]) || formatLabel(displayCategory.toString());
    const displayMessage = formatLabel(iss.user_message || iss.message || iss.title || (iss as any).description || 'Issue');
        ...
        >
          <div className="flex items-start justify-between gap-2">
            <div className="flex flex-col min-w-0 flex-1 gap-0.5">
              ...
              <span className="ppp-issues-row-title text-xs font-semibold text-[var(--text-primary)] line-clamp-2 leading-tight">
                {displayMessage}
              </span>
    ...
    return (
      <section className="ppp-issues-panel">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                ...
                <ul className="ml-4 mt-1">
                  {(autoFixBefore!.categorySummaries || []).slice(0, 5).map(cat => (
                    <li key={cat.category} className="text-sm">
                      {ISSUE_CATEGORY_LABELS[cat.category as keyof typeof ISSUE_CATEGORY_LABELS] || formatLabel(cat.category.toString())}: {(cat.errors || 0) + (cat.warnings || 0) + (cat.info || 0)} issues
                    </li>
                  ))}
                </ul>
              </div>
          ...
          {/* After card */}
          <div className="p-4 bg-green-50/10 border border-green-200/20 rounded-lg">
              ...
              <ul className="space-y-1 max-h-40 overflow-y-auto">
                {diff.issueChanges.fixedIssues.slice(0, 10).map((issue, i) => (
                  <li key={i} className="text-sm text-[var(--text-secondary)]">
                    p.{issue.page} • {ISSUE_CATEGORY_LABELS[issue.category as keyof typeof ISSUE_CATEGORY_LABELS] || formatLabel(issue.category.toString())} • {formatLabel(issue.severity)} — {formatLabel(issue.message)}
                  </li>
                ))}
            ...
            <div>
              ...
              <ul className="space-y-1 max-h-40 overflow-y-auto">
                ...
                {diff.issueChanges.remainingIssues.length > 10 && (
                  <li className="text-sm text-[var(--text-muted)]">... and {diff.issueChanges.remainingIssues.length - 10} more</li>
                )}
              </ul>
            </div>
          )}
  ...
  return (
    <section className="ppp-issues-panel">
      ...
      <div className="ppp-issues-list">
        {counts.total === 0 && (
          <div className="ppp-issues-empty rounded-lg border border-[var(--border-color)] bg-[var(--bg-tertiary)] px-3 py-2 text-sm text-[var(--text-secondary)] text-center absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3/4">
