...
export function InvestorDemo({ onBack, onJobComplete, isAuthenticated }: InvestorDemoProps) {
    ...
    useEffect(() => {
        ...
        pposFetch<{ ok: boolean, policies: any[] }>('/api/v2/preflight/policies')
            ...
            .catch(err => {
                console.error('[DEMO-POLICIES-ERROR]', err);
            });
    }, [isAuthenticated]);
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            pposFetch<any>(`/api/v2/preflight/jobs/${jobId}`)
                .then(res => {
                    const job = res.job || res; // Flexible parsing
                    if (job) {
    ...
    }, [jobId, onJobComplete, isAuthenticated]);
    ...
    const startDemo = async () => {
        ...
        fd.append('pdf', file);
        const selectedProfile = selectedPolicy; 
        fd.append('policy', selectedProfile);

        try {
            const data = await pposFetch<any>('/api/v2/preflight/analyze', { 
                method: 'POST', 
                body: fd 
            });
